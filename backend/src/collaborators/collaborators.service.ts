...
export class CollaboratorsService {
  ...
  async getCollaborators(boardId: string): Promise<BoardCollaborator[]> {
    ...
    for (const collab of (data as BoardCollaborator[]) || []) {
      ...
      collaborators.push({
        ...collab,
        user: (profile as BoardCollaborator['user']) || undefined,
      });
    }
  ...
  async addCollaborator(
    ...
    if (existing) {
      throw new ConflictException(
        'User is already a collaborator on this board',
      );
    }
    ...
    const inviterProfileTyped = inviterProfile as {
      username: string;
      full_name?: string;
    } | null;
    const boardTyped = board as { title: string };
    ...
    const { error: notifError } = await adminSupabase
      ...
      .insert({
        ...
        data: {
          board_id: boardId,
          board_title: boardTyped.title,
          invited_by: invitedBy,
          inviter_name:
            inviterProfileTyped?.full_name ||
            inviterProfileTyped?.username ||
            'Someone',
        },
        read: false,
      });
  ...
  async removeCollaborator(
    ...
    if (board.user_id !== requesterId && userId !== requesterId) {
      throw new ForbiddenException(
        'Not authorized to remove this collaborator',
      );
    }
    ...
    if (userId !== requesterId) {
      ...
      await adminSupabase.from('notifications').insert({
        ...
        data: {
          ...
          remover_name:
            ownerProfileTyped?.full_name ||
            ownerProfileTyped?.username ||
            'Board owner',
        },
        read: false,
      });
  ...
  async updateCollaboratorRole(
    ...
    if (board.user_id !== requesterId) {
      throw new ForbiddenException(
        'Only the board owner can change collaborator roles',
      );
    }
  ...
  async getSharedBoards(userId: string): Promise<SharedBoard[]> {
    ...
    const collabsTyped = collabs as {
      board_id: string;
      role: string;
      status: 'pending' | 'accepted';
    }[];
    const boardIds = collabsTyped.map((c) => c.board_id);
    const { data: boards, error: boardsError } = await supabase
      .from('boards')
      .select(
        'id, title, description, color, created_at, last_opened_at, user_id',
      )
      .in('id', boardIds)
    ...
    for (const board of (boards as BoardRecord[]) || []) {
      const { data: ownerProfile } = await supabase
        .from('profiles')
        .select('id, username, full_name, avatar_url')
        .eq('id', board.user_id)
      ...
      const collab = collabsTyped.find((c) => c.board_id === board.id);
      ...
      sharedBoards.push({
        ...
        owner: (ownerProfile as SharedBoard['owner']) || undefined,
